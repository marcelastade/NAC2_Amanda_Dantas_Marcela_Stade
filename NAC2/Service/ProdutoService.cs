...
namespace NAC2.Service
{
    public class ProdutoService : IProdutoService
    {
        ...
        {
            var produtos = await _repository.GetProdutosAbaixoEstoqueMinimoAsync();

            if (produtos != null && produtos.Any())
            {
                _logger.LogWarning("Existem {Count} produtos abaixo da quantidade mínima em estoque!", produtos.Count());
            }
            else
            {
                _logger.LogInformation("Todos os produtos estão com estoque acima do mínimo.");
            }
        ...
        public async Task<int> AddProdutoAsync(Produto produto)
        {
            ...
            {
                throw new ArgumentException("A categoria deve ser PERECIVEL ou NAO_PERECIVEL.");
            }

            int novoId = await _repository.AddProdutoAsync(produto);
            return novoId;
        }
